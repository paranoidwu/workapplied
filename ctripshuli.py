...
#for d in ["20160808","20160809","20160810","20160811","20160812","20160813","20160814"]:
	fin = open("C:\\Users\\wuenyu\\save\\OTA\\"+d+"\\"+d+"\\galaxy_ota_stat_upperleft_first_group_plan_xiecheng",'r')
	mark = 0
	data[d] = {}
#读取文件并修正问题
	for line in fin:
		...
		if OTA.check_line_item(line) == 10:
			...
			if data[d][mark][9] in ["旅游-单线路","旅游-通用词"] and int(data[d][mark][1]) not in [18669566,18669561,18669559,18371715,18360413,18199240,7414539]:
				print "date %r,%r has wrong accountid"%(d,mark)
...
for d in OTA.timeseries(startdate,enddate):
	...
	for j in range(lenth):
		if data[d][j][9] == "null":
			...
			write_line += 1
savename = startdate+"-"+enddate+"高端定制样式数据报表-携程"+".xls"
